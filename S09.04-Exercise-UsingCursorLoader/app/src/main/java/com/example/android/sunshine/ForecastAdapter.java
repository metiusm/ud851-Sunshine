...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        String weatherForThisDay =
                "Date: " + SunshineDateUtils.getFriendlyDateString(mContext, mCursor.getLong(mCursor.getColumnIndex(COLUMN_DATE)), true) + "\n" +
                "Description: " + SunshineWeatherUtils.getStringForWeatherCondition(mContext, mCursor.getInt(mCursor.getColumnIndex(COLUMN_WEATHER_ID))) + "\n" +
                "Low: " + mCursor.getLong(mCursor.getColumnIndex(COLUMN_MIN_TEMP)) + "\n" +
                "High: " + mCursor.getLong(mCursor.getColumnIndex(COLUMN_MAX_TEMP));
...
//      CPOMPLETED (9) Delete the current body of getItemCount
//      CPOMPLETED (10) If mCursor is null, return 0. Otherwise, return the count of mCursor
